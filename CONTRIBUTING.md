# Contributing to allneurons-minimike
...
- `CODEOWNERS` requires an allNeurons review on every PR that touches `mini-mike/` (or any future plugin folder).
...
- Until a PR is merged, it does not affect anyone who installs `mini-mike@allneurons-minimike` from this repo.
...
1. Create `mini-mike/skills/<skill-name>/SKILL.md`.
...
Agents live under `mini-mike/agents/<name>.md` — the folder exists but is empty in v0.1. When the first agent is added, use frontmatter with `name`, `description`, `model`, and `tools`, a defined schedule (cron-style or "on-demand"), and a body describing what it watches and what it does when triggered.
...
- Bump `mini-mike/.claude-plugin/plugin.json` → `version` on any material change: patch for behavior fixes, minor for a new skill or a new required input.
